...
import org.usfirst.frc2797.Robot2019v2.CustomTimer;
//region Imports
...
import org.usfirst.frc2797.Robot2019v2.subsystems.Limelight;
...
public class TeleopDrive extends Command {
    ...
    protected void execute() {
        ...
        //Move Elevator Down
        if(xbx.getTriggerAxis(left) > 0.2){
            elevator.moveElevator(xbx.getTriggerAxis(left)*.3);
            //System.out.println("right move");
        }
        //Move Elevator Up
        if(xbx.getTriggerAxis(right) > 0.2){
            elevator.moveElevator(-xbx.getTriggerAxis(right));
            //System.out.println("left move");
        }
        //Stop Elevator
        if(xbx.getTriggerAxis(right) <= 0.2 && xbx.getTriggerAxis(left) <= 0.2){
            elevator.moveElevator(-xbx.getTriggerAxis(right));
        }
